...
void CodeGenLLVM::emit_struct_meta_base(const std::string &name,
  ...
  if (snode->type == SNodeType::dense || snode->type == SNodeType::bit_pack) {
    auto body_type =
...
void CodeGenLLVM::visit(GlobalStoreStmt *stmt) {
  ...
  if (ptr_type->is_bit_pointer()) {
    ...
    auto get_ch = stmt->dest->as<GetChStmt>();
    if (!pointee_type->is<CustomIntType>()) {
      // if (get_ch && get_ch->input_snode->type ==
      ...
      // } else
      if (get_ch && get_ch->input_snode->type == SNodeType::bit_array) {
        TI_ERROR("Bit array only supports custom int type.");
      } else if (get_ch && (get_ch->input_snode->type == SNodeType::bit_pack || get_ch->input_snode->type == SNodeType::bit_struct)) {
        // TI_INFO("store cft to bit pack");
        TI_ASSERT(!get_ch->output_snode->owns_shared_exponent);
        auto *cft = pointee_type->as<CustomFloatType>();
        if (cft->get_exponent_type() == nullptr) {
          // for fixed point numbers
          // TI_INFO("store fixed-point to bit pack");
          auto cit = cft->get_digits_type()->as<CustomIntType>();
          auto val = custom_type_to_bits(llvm_val[stmt->val], cft, cit->get_physical_type());
          // TI_INFO("physcial type: {}", cit->get_physical_type()->to_string());
          // create_call("print_value", {get_context(), val});
          auto bit_offset = get_ch->output_snode->bit_offset;
          store_custom_int(llvm_val[stmt->dest], cit, val, bit_offset, true);
        } else {
          TI_ERROR("custom float with exponents are not supported now");
        }
      }
    } else {
      // TI_INFO("store cit to bit pack");
      ...
      // create_call("print_value", {get_context(), store_value});
      store_value = custom_type_to_bits(store_value, cit, cit->get_physical_type());
      store_custom_int(llvm_val[stmt->dest], cit, store_value, bit_offset, true);
    }
  } else {
    builder->CreateStore(llvm_val[stmt->val], llvm_val[stmt->dest]);
  }
  // create_call("print_value", {get_context(), llvm_val[stmt->val]});
...
void CodeGenLLVM::visit(GlobalLoadStmt *stmt) {
  ...
  if (ptr_type->is_bit_pointer()) {
    ...
    if (val_type->is<CustomIntType>()) {
      llvm_val[stmt] = load_custom_int(stmt->src);
...
std::string CodeGenLLVM::get_runtime_snode_name(SNode *snode) {
  if (snode->type == SNodeType::root) {
    ...
    return "BitStruct";
  } else if (snode->type == SNodeType::bit_pack) {
    return "BitPack";
...
void CodeGenLLVM::visit(SNodeLookupStmt *stmt) {
  ...
  if (snode->type == SNodeType::root) {
    ...
    llvm_val[stmt] = call(snode, llvm_val[stmt->input_snode], "lookup_element",
                          {llvm_val[stmt->input_index]});
  } else if (snode->type == SNodeType::bit_struct || snode->type == SNodeType::bit_pack) {
...
void CodeGenLLVM::visit(GetChStmt *stmt) {
  if (stmt->input_snode->type == SNodeType::bit_array) {
    ...
    if (bit_struct->is_bit_pack) {
      int world_length = arch_is_cpu(prog->config.arch) ? 8 : 8;
      // TI_INFO("world_length: {}", world_length);
      int word_offset = stmt->output_snode->word_offset;
      // TI_INFO("creating bit pointer: {} word offset, {} bit offset", word_offset, bit_offset);
      llvm_val[stmt->input_ptr] = builder->CreateGEP(
        builder->CreateBitCast(llvm_val[stmt->input_ptr],llvm::PointerType::getInt8PtrTy(*llvm_context)),
        tlctx->get_constant(word_offset*world_length));
    }
...
void CodeGenLLVM::visit(BackendTestStmt *stmt) {
  llvm_val[stmt]= create_call("print_sth");
}
