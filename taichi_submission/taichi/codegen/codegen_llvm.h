...
class CodeGenLLVM : public IRVisitor, public LLVMModuleBuilder {
 public:
  ...
  void store_split(llvm::Value* bit_struct_val,
                    llvm::Value *byte_ptr,
                    CustomIntType *cit,
                    int bit_offset,
                    bool is_atomic);

  llvm::Value* load_split(llvm::Value *byte_ptr,
                            CustomIntType *cit,
                            int bit_offset,
                            bool is_atomic);
  ...
  void visit(BitPackAssignStmt *stmt) override;
  ...
  virtual llvm::Value *load_as_custom_int(llvm::Value *ptr, int bit_offset, Type *load_type);

  llvm::Value *get_mask_constant(DataType dt);

  llvm::Value *extract_custom_int(llvm::Value *physical_value,
                                  llvm::Value *bit_offset,
                                  Type *load_type);

  llvm::Value *extract_custom_int_by_mask(llvm::Value *physical_value,
                                  llvm::Value *bit_offset,
                                  Type *load_type);
  ...
  llvm::Value *load_custom_int(Stmt *ptr_stmt);
  ...
  void visit(BackendTestStmt* stmt) override;
  ...
  virtual llvm::Value *create_load(llvm::Value* ptr, const DataType &dtype);
