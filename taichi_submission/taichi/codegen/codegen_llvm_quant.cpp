...
llvm::Value *CodeGenLLVM::float_to_custom_int(CustomFloatType *cft,
  ...
  o = builder->CreateFPCast(
      llvm::ConstantFP::get(*llvm_context, llvm::APFloat(cft->get_offset())),
      llvm_type(compute_type));
  auto input_real = builder->CreateFPCast(real, llvm_type(compute_type));
  auto centered = builder->CreateFSub(input_real, o);
  auto scaled = builder->CreateFMul(centered, s);
  // scaled = builder->CreateFSub(scaled, o);

  // Add/minus the 0.5 offset for rounding
  if (cft->get_use_dithering()) {
    scaled = create_call(fmt::format("rounding_prepare_f{}_with_dithering",
                                     data_type_bits(compute_type)),
                         {get_context(), scaled});
  } else {
    scaled = create_call(
...
void CodeGenLLVM::store_custom_int(llvm::Value *bit_ptr,
  ...
  auto num_bits = cit->get_num_bits();
  auto physical_type_bits = data_type_bits(cit->get_physical_type());
  
  store_value = value;

  if (num_bits + bit_offset > physical_type_bits) {
    auto [byte_ptr, _bit_offset] = load_bit_pointer(bit_ptr);
    // create_call("print_address", {get_context(), byte_ptr});
    store_split(store_value, byte_ptr, cit, bit_offset, atomic);
  } else {
    store_custom_int(bit_ptr, cit, store_value, atomic);
  }
...
void CodeGenLLVM::visit(BitStructStoreStmt *stmt) {
  ...
  if (store_all_components && !has_shared_exponent && bit_struct_snode->type != SNodeType::bit_pack) {
...
llvm::Value *CodeGenLLVM::load_as_custom_int(llvm::Value *ptr,
  ...
  auto v = extract_custom_int(bit_level_container, bit_offset, load_type);
  return v;
}


llvm::Value *CodeGenLLVM::load_as_custom_int(llvm::Value *ptr, int bit_offset, Type *load_type) {
    auto *cit = load_type->as<CustomIntType>();
    auto num_bits = cit->get_num_bits();
    auto [byte_ptr, _bit_offset] = load_bit_pointer(ptr);

    if (num_bits + bit_offset > 64) {
      // TI_INFO("load split");
      return load_split(byte_ptr, cit, bit_offset, false);
    } 

    auto dtype = cit->get_physical_type();
    byte_ptr = builder->CreateBitCast(byte_ptr, llvm_ptr_type(dtype));
    auto bit_level_container = create_load(byte_ptr, dtype);

    return extract_custom_int_by_mask(bit_level_container, _bit_offset, load_type);
}
...
llvm::Value *CodeGenLLVM::get_mask_constant(DataType dt) {
  if (data_type_bits(dt) == 32) return tlctx->get_constant(~(0));
  else if (data_type_bits(dt) == 64) return tlctx->get_constant(~0UL);
  else {
    TI_ERROR("Please set correct type!");
  }
}

llvm::Value *CodeGenLLVM::extract_custom_int_by_mask(llvm::Value *physical_value,
                                             llvm::Value *bit_offset,
                                             Type *load_type) {

  auto cit = load_type->as<CustomIntType>();

  auto physical_length = data_type_bits(cit->get_physical_type());

  auto _bit_offset = builder->CreateIntCast(bit_offset, llvm_type(cit->get_physical_type()), false);
  
  auto msb_loc = builder->CreateAdd(_bit_offset, tlctx->get_constant(cit->get_physical_type(), cit->get_num_bits()));
  auto high_rest = builder->CreateSub(tlctx->get_constant(cit->get_physical_type(), physical_length), msb_loc);


  // step1: Get the value of the target cit by mask.
  auto mask = builder->CreateShl(get_mask_constant(cit->get_physical_type()), physical_length - cit->get_num_bits());
  mask = builder->CreateLShr(mask, high_rest);
  auto value = builder->CreateAnd(mask, physical_value);
  value = builder->CreateLShr(value, _bit_offset);

  if (!cit->get_is_signed()) {
    // for unsigned numbers, simply return the masked value;
    return builder->CreateIntCast(value, llvm_type(cit->get_compute_type()), false);;
  } else {
    // step2: Get the sign of the number.
    auto sign_bit = cit->get_num_bits() - 1;
    auto sign = builder->CreateLShr(value, sign_bit);
    sign = builder->CreateAnd(sign, tlctx->get_constant(cit->get_physical_type(), 1));

    // step3: compose the highest part of the number
    // for numbers < 0, the highest part is '1' and '0' for numbers > 0. 
    auto high_mask = builder->CreateShl(tlctx->get_constant(cit->get_physical_type(), 1), physical_length - 1);
    high_mask = builder->CreateAShr(high_mask, high_rest);
    auto sign_to_highest_bit = builder->CreateShl(sign, physical_length - 1);
    auto sign_to_sign_bit = builder->CreateAShr(sign_to_highest_bit, tlctx->get_constant(cit->get_physical_type(), physical_length - cit->get_num_bits()));
    
    value = builder->CreateOr(sign_to_sign_bit, value);
    return builder->CreateIntCast(value, llvm_type(cit->get_compute_type()), true);
  }
}
...
llvm::Value *CodeGenLLVM::reconstruct_custom_float(llvm::Value *digits,
  ...
  auto t = builder->CreateFMul(cast, s);
  llvm::Value *o = llvm::ConstantFP::get(*llvm_context, llvm::APFloat(cft->get_offset()));
  o = builder->CreateFPCast(o, llvm_type(compute_type));
  return builder->CreateFAdd(t, o);
}
...
llvm::Value *CodeGenLLVM::load_custom_float(Stmt *ptr_stmt) {
  ...
  if (cft->get_exponent_type()) {
    ...
    if (ptr->input_snode->type == SNodeType::bit_pack) {
      TI_ERROR("custom float with exponents are not supported on bit_pack");
    }
    ...
    auto bit_offset = ptr->output_snode->bit_offset;
    auto digits = load_as_custom_int(llvm_val[ptr], bit_offset, cft->get_digits_type());
    return reconstruct_custom_float(digits, cft);
  }
}

llvm::Value *CodeGenLLVM::load_custom_int(Stmt *ptr_stmt) {
  auto ptr = ptr_stmt->as<GetChStmt>();
  TI_ASSERT(ptr);
  auto cit = ptr->ret_type->as<PointerType>()
                 ->get_pointee_type()
                 ->as<CustomIntType>();
  TI_ASSERT(cit);

  auto _bit_offset = ptr->output_snode->bit_offset;
  return load_as_custom_int(llvm_val[ptr], _bit_offset, cit);
}

void CodeGenLLVM::store_split(llvm::Value* store_val, 
                                llvm::Value *byte_ptr,
                                CustomIntType *cit, 
                                int bit_offset,
                                bool is_atomic) {

  store_val = builder->CreateIntCast(store_val, llvm::Type::getInt64Ty(*llvm_context), true);

  auto* v1 = builder->CreateShl(store_val, bit_offset);

  auto end_bits = bit_offset + cit->get_num_bits();

  auto d = end_bits - 64;

  uint64 mask = 0;
  update_mask(mask, cit->get_num_bits()-d, bit_offset);


  auto bit_struct_physical_type = cit->get_physical_type(); 
  store_masked(byte_ptr, mask, bit_struct_physical_type, v1, is_atomic);

  mask = 0;
  update_mask(mask, d, (cit->get_num_bits()-d));
  
  auto *v2 = builder->CreateAnd(store_val, mask);

  v2 = builder->CreateLShr(v2, (cit->get_num_bits()-d));
  auto* ptr = builder->CreateGEP(
          builder->CreateBitCast(byte_ptr, llvm::PointerType::getInt8PtrTy(*llvm_context)),
          tlctx->get_constant(8)
        );
  mask = 0;
  update_mask(mask, d, 0);
  store_masked(ptr, mask, bit_struct_physical_type, v2, is_atomic);
}

llvm::Value* CodeGenLLVM::load_split(llvm::Value *byte_ptr,
                                      CustomIntType *cit, 
                                      int bit_offset,
                                      bool is_atomic) {

  auto* v1 = create_load(builder->CreateBitCast(byte_ptr, llvm_ptr_type(cit->get_physical_type())), cit->get_physical_type());
  auto* ptr = builder->CreateGEP(
              builder->CreateBitCast(byte_ptr, llvm::PointerType::getInt8PtrTy(*llvm_context)),
              tlctx->get_constant(8));

  auto* v2 = create_load(builder->CreateBitCast(ptr, llvm_ptr_type(cit->get_physical_type())), cit->get_physical_type());

  auto end_bits = bit_offset + cit->get_num_bits();
  auto d = end_bits - 64;
  // TI_INFO("bit_offset: {}", bit_offset);
  // TI_INFO("num_bits: {}", cit->get_num_bits());
  // TI_INFO("end_bits: {}", end_bits);
  // TI_INFO("d: {}", d);
  uint64 mask = 0;
  update_mask(mask, cit->get_num_bits()-d, bit_offset);
  // TI_INFO("mask: {:x}", mask);
  auto low = builder->CreateAnd(v1, tlctx->get_constant(mask));

  // create_call("print_value", {get_context(), v1});
  // create_call("print_value", {get_context(), v2});
  // create_call("print_value", {get_context(), low});

  mask = 0;
  update_mask(mask, d, 0);
  auto high = builder->CreateAnd(v2, tlctx->get_constant(mask));
  low = builder->CreateLShr(low, bit_offset);
  if (cit->get_is_signed()) {
    high = builder->CreateShl(high, 64-d);
    high = builder->CreateAShr(high, (64-cit->get_num_bits()));
  } else {
    high = builder->CreateShl(high, cit->get_num_bits()-d);
  }
  auto* v = builder->CreateOr(high, low);

  // create_call("print_value", {get_context(), high});
  // create_call("print_value", {get_context(), low});

  return builder->CreateIntCast(v, llvm_type(cit->get_compute_type()),
                                cit->get_is_signed());
}

llvm::Value *CodeGenLLVM::create_load(llvm::Value* ptr, const DataType &dtype) {
  return builder->CreateLoad(ptr);
}

void CodeGenLLVM::visit(BitPackAssignStmt* stmt) {
  auto* bit_pack_snode = stmt->get_bit_pack_snode();
  auto bit_pack_snode_physical_type = bit_pack_snode->physical_type;
  auto physical_type_bits = data_type_bits(bit_pack_snode_physical_type);
  TI_ASSERT(bit_pack_snode->type == SNodeType::bit_pack);

  llvm::Value * physical_value = nullptr;
  llvm::Value * val = nullptr;
  llvm::Value * higher_bits = nullptr;
  bool splited = false;
  auto ptr = llvm_val[stmt->ptr];
  auto ptr_current = ptr;
  TI_ASSERT(bit_pack_snode->ch.size() == stmt->values.size());

  for (int i = 0; i < stmt->values.size(); i++) {
    auto& ch = bit_pack_snode->ch[i];
    auto dtype = ch->dt;

    auto num_bits = 0;
    if (dtype->is<CustomFloatType>()) {
      auto* cft = dtype->as<CustomFloatType>();
      TI_ASSERT_INFO(cft->get_exponent_type() == nullptr, "BitPackAssignStmt only supports fixed points for now");
      num_bits = cft->get_digit_bits() + (int)cft->get_is_signed();
    } else if (dtype->is<CustomIntType>()) {
      auto* cit = dtype->as<CustomIntType>();
      num_bits = cit->get_num_bits();
    }
    TI_ASSERT(num_bits != 0);
   
    auto bit_offset = ch->bit_offset;
    auto word_offset = ch->word_offset;
    
    auto arg = stmt->values[i];
    val = custom_type_to_bits(llvm_val[arg], dtype, bit_pack_snode_physical_type);

    // TODO: decide 64 or 32 acorrding to physical types.
    if (bit_offset + num_bits >= physical_type_bits) {
      // create a mask
      auto t = bit_offset + num_bits - physical_type_bits;
      auto m = physical_type_bits - bit_offset;
      auto mask = (uint64)((1<<t)-1) << m;

      // record the bits in the next physical types.
      higher_bits = builder->CreateAnd(tlctx->get_constant(mask), val);
      higher_bits = builder->CreateAShr(higher_bits, tlctx->get_constant<uint64>(m));
      splited = true;
    }

    val = builder->CreateShl(val, bit_offset);
    if (physical_value == nullptr) {
      physical_value = val;
    } else {  
      physical_value = builder->CreateOr(val, physical_value);
    }

    if (splited) {
      splited = false;
      auto dest = builder->CreateBitCast(ptr_current, llvm_ptr_type(bit_pack_snode_physical_type));
      builder->CreateStore(physical_value, dest);
      physical_value = higher_bits;
      ptr_current = builder->CreateGEP(
          builder->CreateBitCast(ptr, llvm::PointerType::getInt8PtrTy(*llvm_context)),
          tlctx->get_constant(8 * (word_offset + 1)));
    }
  }
  auto dest = builder->CreateBitCast(ptr_current, llvm_ptr_type(bit_pack_snode_physical_type));
  llvm_val[stmt] = builder->CreateStore(physical_value, dest);
