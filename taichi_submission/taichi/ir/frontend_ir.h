...
class FrontendBitPackAssignStmt : public Stmt {
 public:
  SNode *snode;
  ExprGroup indices;
  std::vector<Expr> args;
  FrontendBitPackAssignStmt(SNode *snode, ExprGroup indices, const std::vector<Expr> &args_)
    :snode(snode), indices(indices) {
    TI_ASSERT(snode->type == SNodeType::bit_pack);
    for (auto &a : args_) {
      args.push_back(load_if_ptr(a));
    }
  }
