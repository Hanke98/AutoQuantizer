...
SNode &SNode::insert_children(SNodeType t) {
  ...
  if (type == SNodeType::bit_struct || type == SNodeType::bit_array || type == SNodeType::bit_pack) {
...
SNode &SNode::bit_pack(bool packed) {
  auto &snode = create_node({}, {}, SNodeType::bit_pack, packed);
  snode.physical_type =
...
std::string SNode::get_node_type_name_hinted() const {
  std::string suffix;
  if (type == SNodeType::place || type == SNodeType::bit_struct || type == SNodeType::bit_pack ||
