...
class BitPackAssignStmt : public Stmt {
 public:
  Stmt *ptr;
  std::vector<Stmt *> values;

  BitPackAssignStmt(Stmt *ptr, const std::vector<Stmt *> &values)
    : ptr(ptr), values(values) {
    TI_STMT_REG_FIELDS;
  }
  
  SNode *get_bit_pack_snode() const;
  
  bool common_statement_eliminable() const override {
    return false;
  }

  TI_STMT_DEF_FIELDS(ret_type, ptr, values);
  TI_DEFINE_ACCEPT_AND_CLONE;
};

class BackendTestStmt : public Stmt {
 public:

  BackendTestStmt() {
    TI_INFO("BackendTestStmt constructor");
    TI_STMT_REG_FIELDS;
  };
