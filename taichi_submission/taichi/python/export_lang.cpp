...
#undef PER_EXTENSION
  ...
  py::class_<SNode>(m, "SNode")
      ...
      .def("bit_pack", &SNode::bit_pack, py::return_value_policy::reference)
...
#undef PER_TYPE
  ...
  m.def("bit_pack_assign", [&](SNode *snode, const ExprGroup &indices, const std::vector<Expr>& args) {
     current_ast_builder().insert(std::make_unique<FrontendBitPackAssignStmt>(snode, indices, args)); 
  });
  m.def("test_front", []() {
    current_ast_builder().insert(std::make_unique<BackendTestStmt>());
   });
  ...
  py::class_<TypeFactory>(m, "TypeFactory")
      ...
      .def("get_custom_float_type", &TypeFactory::get_custom_float_type,
           ...
           py::arg("compute_type"), py::arg("scale"), py::arg("offset"),
           py::arg("use_dithering"), py::return_value_policy::reference);
