...
#define ROUND_PREPARE_WITH_DITHERING(N)                                   \
  f##N rounding_prepare_f##N##_with_dithering(Context *context, f##N f) { \
    f = f + rand_f##N(context) - (f##N)0.5;                               \
    return rounding_prepare_f##N(f);                                      \
  }

ROUND_PREPARE_WITH_DITHERING(32)
ROUND_PREPARE_WITH_DITHERING(64)

void print_address(Context *context, u8* ptr) {
  taichi_printf((LLVMRuntime *)context->runtime, "base ptr: %x\n", ptr);
}

void print_value(Context *context, u64 value) {
  taichi_printf((LLVMRuntime *)context->runtime, "value: 0x%016lx\n", value);
  // taichi_printf((LLVMRuntime *)context->runtime, "value:\n" );
}

void print_value_f32(Context *context, f32 value) {
  taichi_printf((LLVMRuntime *)context->runtime, "value: %f\n", value);
  // taichi_printf((LLVMRuntime *)context->runtime, "value:\n" );
}


void print_data(Context *context, u64 value, const char* annot) {
  taichi_printf((LLVMRuntime *)context->runtime, "%s: 0x%016lx\n", annot, value);
}

void print_mask(Context *context, u64 value) {
  taichi_printf((LLVMRuntime *)context->runtime, "maks: 0x%016lx\n", value);
}
