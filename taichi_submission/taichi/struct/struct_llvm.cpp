...
void StructCompilerLLVM::generate_types(SNode &snode) {
  ...
  if (type == SNodeType::dense || type == SNodeType::bitmasked) {
  ...
  } else if (type == SNodeType::bit_struct || type == SNodeType::bit_pack) {
    // Generate the bit_struct type or bit_pack type
    std::vector<Type *> ch_types;
    std::vector<int> ch_offsets;
    int total_offset = 0;
    // TI_INFO("ch size: {}", snode.ch.size());
    for (int i = 0; i < snode.ch.size(); i++) {
      auto &ch = snode.ch[i];
      ...
      // Set the default physical type of bit pack to 64 bits for now.
      auto aligment_bits = 64;
      ch->word_offset = type == SNodeType::bit_pack? (total_offset/aligment_bits):0;
      ch->bit_offset = type == SNodeType::bit_pack? (total_offset - (ch->word_offset)*aligment_bits):total_offset;
      ch_offsets.push_back(ch->bit_offset);
      // TI_INFO("total_offset: {}, word_offset: {}, bit_offset: {}", total_offset, ch->word_offset, ch->bit_offset);
      ch->total_num_bits = component_cit->get_num_bits();
    ...
    snode.dt = TypeFactory::get_instance().get_bit_struct_type(
        snode.physical_type, ch_types, ch_offsets, type == SNodeType::bit_pack);
    DataType container_primitive_type(snode.physical_type);
    body_type = tlctx_->get_data_type(container_primitive_type);
    if (type == SNodeType::bit_pack) {
      body_type = llvm::ArrayType::get(body_type, (total_offset+63)/64);
    } 
