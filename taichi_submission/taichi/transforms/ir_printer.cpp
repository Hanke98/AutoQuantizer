...
class IRPrinter : public IRVisitor {
 public:
  ...
  void visit(FrontendBitPackAssignStmt *assign) override {
    std::string values;
    for (auto& a: assign->args) {
      values += " ";
      values += a.serialize();
    }
    print("Frontend bit_pack assign: {}", values);
  }
  ...
  void visit(BitPackAssignStmt * stmt) override {
    print("bit pack assign!");
  }
  void visit(BackendTestStmt* stmt) override {
    print("BackendTestStmt ir");
  }
};
