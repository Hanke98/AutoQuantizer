 ...
 public:
  ...
  void visit(FrontendForStmt *stmt) override {
    ...
    if (stmt->is_ranged()) {
      ...
      // higher performance.
      if (snode->type == SNodeType::bit_struct || snode->type == SNodeType::bit_pack)
        snode = snode->parent;
  ...
  void visit(FrontendBitPackAssignStmt *stmt) override {
    // expand rhs
    Stmt *val_stmt = nullptr;
    auto fctx = make_flatten_ctx();
    ...
    // TI_INFO("snode name: {}", stmt->snode->get_node_type_name());
    std::vector<Stmt *> indices_stmt(stmt->indices.size(), nullptr);
    ...
    auto ptr = fctx.push_back<GlobalPtrStmt>(stmt->snode, indices_stmt);
    fctx.push_back<BitPackAssignStmt>(ptr, args_stmts);
    stmt->parent->replace_with(stmt, std::move(fctx.stmts));
    throw IRModified();
  }
