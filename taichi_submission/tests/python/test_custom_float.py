...
@pytest.mark.parametrize('use_bit_pack', [True, False])
@ti.test(require=ti.extension.quant_basic)
def test_custom_matrix_rotation(use_bit_pack):

    if use_bit_pack:
        cft = ti.quant.fixed(frac=28, range=1.2)
        x = ti.Matrix.field(2, 2, dtype=cft)
        ti.root.bit_pack().place(x.get_scalar_field(0, 0), x.get_scalar_field(0, 1), x.get_scalar_field(1, 0), x.get_scalar_field(1, 1))
    else:
